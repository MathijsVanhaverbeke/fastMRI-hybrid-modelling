...
print('Libraries imported. Starting to load the overfitting dataset consisting of 1 unaugmented scan of 16 slices...')
...
file_paths_train = sorted(glob.glob(path_to_save_mri_data+"training_data_DeepMRIRec_16_coils_batch_1.npy"))
file_paths_train_GT = sorted(glob.glob(path_to_save_mri_data+"training_data_GT_DeepMRIRec_16_coils_batch_1.npy"))
...
def train_generator(file_paths_train, file_paths_train_GT):
    for file_path_train, file_path_train_GT in zip (file_paths_train, file_paths_train_GT):
        with lock:
            # Crop the scan to the unaugmented slices
            x_train = np.load(file_path_train)[:16]
            y_train = np.load(file_path_train_GT)[:16]

            # Yield batches of 8 slices
            for i in range(0, len(x_train), 8):
                yield (x_train[i:i + 8], y_train[i:i + 8])
...
def get_callbacks(learning_rate_drop=0.7,learning_rate_patience=7, verbosity=1):
    callbacks = list()
    callbacks.append(ReduceLROnPlateau(monitor='loss',factor=learning_rate_drop,patience=learning_rate_patience,verbose=verbosity))
    callbacks.append(EarlyStopping(monitor='loss',verbose=verbosity,patience=30))
    return callbacks
...
history = model.fit(training_dataset,
            epochs=1000,
            shuffle=True,
            callbacks=get_callbacks(0.6,10,1))


model.save_weights("/usr/local/micapollo01/MIC/DATA/STUDENTS/mvhave7/Results/Models/Overfitted_DeepMRIRec_GPU.h5")
