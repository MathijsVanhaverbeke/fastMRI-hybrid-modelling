...
training_dataset = tf.data.Dataset.from_generator(generator=lambda: train_generator(file_paths_train, file_paths_train_GT), output_shapes=((None, None, None, None), (None, None, None, 1)), output_types=(tf.uint8, tf.uint8))
validation_dataset = tf.data.Dataset.from_generator(generator=lambda: validation_generator(file_paths_val, file_paths_val_GT), output_shapes=((None, None, None, None), (None, None, None, 1)), output_types=(tf.uint8, tf.uint8))
...
def model_loss_all(y_true, y_pred):
    ...
    ssim_loss = 1 - tf.reduce_mean(tf.image.ssim(img1=y_true,img2=y_pred,max_val=255,filter_size=3,filter_sigma=0.1))
    pixel_loss = tf.reduce_mean(tf.math.abs(y_true-y_pred))
    
    content_loss = 0.0
    res_y_rss = tf.image.grayscale_to_rgb(y_true)
    res_y_rss = preprocess_input(res_y_rss)
    vgg_f_gt = vgg_model(res_y_rss)

    res_y_pred = tf.image.grayscale_to_rgb(y_pred)
...
def create_gen(gen_ip, nlayers, nbasefilters, drop_rate):
    ...
    # Add ReLU activation layer to force output images to have positive pixel values
    op = Activation('relu')(op)
    # Add clipping activation layer to force output images to have pixel values [0,255]
    #op = clip_by_value(op, 0, 255)
...
model_name = "/usr/local/micapollo01/MIC/DATA/STUDENTS/mvhave7/Results/Models/best_model_DeepMRIRec_trained_in_batches_GPU.h5"
...
model.save_weights("/usr/local/micapollo01/MIC/DATA/STUDENTS/mvhave7/Results/Models/final_model_DeepMRIRec_trained_in_batches_GPU.h5")
